...
last_source_check: 2026-07-13
raw_path: research/raw/2026-07-13/gpt-5-6-system-card/
previous_captures: []
static: true
tags: [openai, gpt-5-6, sol, terra, luna, model-release, system-card, safety, preparedness]
---

# GPT-5.6 System Card

_OpenAI — System Card — dated 2026-07-09_
...
This page is a Markdown rendering of a downloaded PDF (`gpt-5-6.pdf`, 81 pages, "GPT-5.6 Preview System Card", OpenAI, PDF dated 2026-07-09). The immutable original PDF and the plain-text extraction are in `research/raw/2026-07-13/gpt-5-6-system-card/`. Known gaps in this rendition:

- **No pricing.** The system card contains no API pricing for Sol, Terra, or Luna. Any per-token pricing must be sourced elsewhere (e.g. OpenAI's pricing page) and cannot be cited to this document.
- **Benchmark tables are flattened.** `pdftotext` did not preserve table grids, so throughout §3–§9 the numeric values survive but their row/column alignment is lost. Read the original PDF for exact cell alignment before citing any specific benchmark score.
- **Figures not captured.** The card's figures (forecast funnels, monitorability curves, metagaming plots, and the Preparedness charts — referenced as "Figure 1", "Figure 2", … in the body) are raster images in the PDF and are not reproduced here; only their captions are captured.
- **Math / footnote notation partially garbled.** Passages with LaTeX math or superscripts (e.g. the Wald-test / resampling-fidelity notation in §3.1.2 and the g-mean² definitions in §7.3) are inlined imperfectly. Refer to the PDF for exact notation.
- **`source_url` is null** — ingested from a downloaded PDF, not a fetched URL; the canonical publication URL was not captured or verified.
