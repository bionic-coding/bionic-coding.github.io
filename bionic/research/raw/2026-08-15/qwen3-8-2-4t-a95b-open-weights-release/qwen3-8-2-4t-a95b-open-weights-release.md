# Qwen3.8-2.4T-A95B — open-weights release (Hugging Face model card)

- **source_url:** https://huggingface.co/Qwen/Qwen3.8-2.4T-A95B
- **fetched_at:** 2026-08-15
- **note:** The Qwen3.8 open-weights release — 2.4T total / 95B active MoE, weights live on HF (6,381 downloads last month), custom license "qwen3.8-max", full architecture spec and benchmark table. Resolves the wiki's open question "did the Qwen3.8-Max weights ship?".
- **capture_method:** WebFetch (server-rendered markdown). HF site navigation chrome stripped; model card body preserved.

---

## Page metadata (HF chrome)

- Org: Qwen (97.4k followers)
- Model: Qwen/Qwen3.8-2.4T-A95B — like 961, Community 36
- Tags: Text Generation, Transformers, Safetensors, qwen3_5_moe_text, conversational, Eval Results
- **License: qwen3.8-max** (custom license, named after the API model)
- Downloads last month: 6,381
- Model size: **2.4T params**; Tensor type: BF16; Chat template included
- Inference provider listed: Together AI
- Model tree: 2 finetunes, 18 quantizations; 4 Spaces
- Collection: Qwen3.8 (4 items, updated 2 days ago, 242)
- Evaluation results (HF eval-results widget): DeepSWE 56.6 (datacurve/deep-swe); WildClawBench Overall 56.2, Avg Time 708 (internlm/WildClawBench); SWE-bench Pro 67.7 (ScaleAI/SWE-bench_Pro); GPQA Diamond 92.6 (Idavidrein/gpqa); HLE 43.6 (cais/hle)
...
Columns: Opus 4.8 / Fable 5 / GPT 5.6 Sol (max) / Qwen3.7-Max / Qwen3.8-Max
...
- vLLM: Qwen3.8 Recipe (https://recipes.vllm.ai/Qwen/Qwen3.8-2.4T-A95B)
...
Text-only input example (OpenAI SDK, streaming, `reasoning_effort="xhigh"`, `extra_body={"chat_template_kwargs": {"enable_thinking": True, "preserve_thinking": True}}` — full sample code in the model card).
...
1. **Sampling Parameters**: We suggest using the following set of sampling parameters: `temperature=1.0`, `top_p=0.95`, `top_k=20`, `min_p=0.0`, `presence_penalty=0.0`, `repetition_penalty=1.0`. For supported frameworks, you can adjust the `presence_penalty` parameter between 0 and 2 to reduce endless repetition. However, using a higher value may occasionally result in language mixing and a slight decrease in model performance.
