...
## MiMo-V2.6 (Xiaomi) — released and open-sourced 22 September

Source: [[research/sources/mimo-v2-6]] (Xiaomi launch article, `mimo.xiaomi.com/mimo-v2-6/article`, 2026-09-22; the canonical landing URL `mimo.xiaomi.com/mimo-v2-6` is a JS-hydrated shell of the same content). Trust-ladder rung 3 — a vendor launch post with a full benchmark table (transcribed from a companion `bench.js` data file, not the article's own static markup), but **no architecture spec of any kind** — no parameter count, no MoE structure, no context window is stated anywhere in the captured text. This is the first MiMo entry in this wiki; no prior version to check for contradiction.

- **What shipped:** two natively omnimodal models, **MiMo-V2.6-Pro** and **MiMo-V2.6-Flash**, plus an **UltraSpeed** mode for Pro (up to 20× faster output, "same quality" — vendor claim). Framed explicitly as RSI exploration: "scaling RL compute on verifiable, complex tasks, so the model can continuously expand its capability frontier through exploration and feedback." (See [[research/concepts/recursive-self-improvement]] for how this framing sits against the wiki's two prior RSI accounts.)
- **Headline claim:** MiMo-V2.6-Pro scores **46.32 on the Artificial Analysis Intelligence Index v4.3**, "surpassing Kimi K3 and Qwen3.8 Max to become the strongest open-source model to date." This is Xiaomi's own citation of an AA index score, not an AA-authored capture — no independent corroboration is in hand.
- **RL training, in the vendor's own numbers:** 30 RL steps over ~750k trajectories in under six days; cost **~$0.85M (Flash) / ~$2.62M (Pro)**; training-task pass rate up 25% (Flash) / 12% (Pro) relative; on the held-out **DeepSWE v1.1** benchmark, Flash rose 48.8→65.68 and Pro rose 58.4→72.57 over the run. Both runs were live-streamed. Router frozen mid-run "to suppress training drift"; a stated reward-hacking defense spans "reward design, adversarial evaluation, anomaly detection and cross-checking between verifiers."
- **Full benchmark table vs. this wiki's tracked frontier set** (Opus 5, GPT 5.6 Sol, Claude Fable 5, GPT 6 Astra, Claude Fable 5.1) is transcribed verbatim in the source page, section by section (Coding/General/Visual/Cyber). **No row shows a MiMo model beating every closed frontier model simultaneously** — GPT 6 Astra or Claude Fable 5.1 leads most General/Cyber rows. MiMo's own strength within its comparison set concentrates in coding (ties/wins on DeepSWE against every non-frontier open model) and one in-house cyber benchmark (MiMo Cyber Bench, 81.7 — nobody else's benchmark to check). MiMo-V2.6-Flash's best row is CyberGym at 95.1, ahead of its own Pro sibling (94.0). The "strongest open-source model" claim is scoped to the AA Index specifically, not this table.
- **Pricing unchanged from V2.5.** Flash $0.0028/$0.14/$0.28 (cache-hit/cache-miss/output per MTok); Pro $0.0036/$0.435/$0.87; Pro-UltraSpeed $0.036/$4.35/$8.7. Cache writes free "for a limited time."
- **Availability:** AI Studio, MiMo Code, MiMo Desktop (first official release out of early access), the MiMo API Platform, and OpenRouter (`openrouter.ai/xiaomi`).
- **Weights and license — still open, but the architecture gap is now filled.** The article links a **Hugging Face collection** (`huggingface.co/collections/XiaomiMiMo/mimo-v26`) and a **technical-report PDF hosted on HF** (`huggingface.co/XiaomiMiMo/MiMo-V2.6-Pro-RL/blob/main/MiMo_V2_6_technical_report.pdf`) — both recorded as pointers only, **not ingested**. **License terms remain unverified.** Do not repeat "open-sourcing" here without a license caveat until the HF card itself is captured — this is the cheapest next ingest on this page.
- **Architecture, from OpenRouter's listing** ([[research/sources/openrouter-mimo-v2-6-pro]], captured 2026-09-22 — a headless-Chrome render, the live page is JS-rendered): **MiMo-V2.6-Pro is >1T parameters** (total; active-parameter count still not stated anywhere); **MiMo-V2.6-Flash is MoE, 309B total / 15B active per token, hybrid attention**; **Pro-UltraSpeed is the same 1T-class checkpoint as Pro**. Context 1,048,576 tokens / max output 131,072 across the family; pricing on OpenRouter matches Xiaomi's own API pricing exactly ($0.435/$0.87/$0.0036 per MTok for Pro).
- **First independent corroboration of the AA Intelligence Index claim.** Artificial Analysis's own score, surfaced on OpenRouter's model page, is **46.3** — matching Xiaomi's self-cited **46.32** to one decimal place. The full AA table (also transcribed in the source page): HLE 49.4%, AA-LCR 86.3%, GDPval-AA 58.7%, CritPt 26.6%, SciCode 60.9%, AA-Omniscience Accuracy 34.8%, Non-Hallucination Rate 59.4%; OpenRouter's own callout reads "Better than 92% of models compared." This is the first non-Xiaomi-authored figure on MiMo-V2.6 in this wiki — meaningfully stronger than Xiaomi's self-citation of the same index, though it corroborates only the Intelligence Index score, not the rest of Xiaomi's benchmark table.
- **Provider serving data (DeepInfra vs. Xiaomi's own endpoint):** identical pricing on both ($0.435/$0.87/$0.0036 per MTok); DeepInfra faster-to-first-token (1.00s vs 3.87s latency) but lower throughput (24 vs 30 tps) and lower measured uptime (97.83% vs 100.00%) than Xiaomi's own endpoint.
- > [contradiction] **UltraSpeed's speed multiplier disagrees across sources.** Xiaomi's own launch article claims UltraSpeed delivers "up to 20x faster output speed at the same quality." OpenRouter's model description for the same variant says "roughly 10x" Pro's output speed. Not resolved — cite the specific source when repeating either number.
- **Vendor demos beyond benchmarks** (unaudited, single-run, no numbers): "Vibe World" (3D scene generation, Blender modeling, embodied robotic-arm control via multi-view camera feedback), frontend/slide design, end-to-end video production, a first exploration into music composition (orchestral MIDI, piano pieces), and two research case studies — a materials-design MOF search and a >6,000-line Lean 4 formalization of the full Li–Yorke theorem verified by Lean's kernel with no post-training on Lean specifically.
...
- **What does MiMo-V2.6's license permit?** ✅ Partially resolved (2026-09-22) — OpenRouter's listing fills the architecture gap (Pro >1T params; Flash 309B/15B MoE; context 1,048,576/output 131,072 — see [[research/sources/openrouter-mimo-v2-6-pro]]), but license terms are still unstated anywhere captured. They live only in the uncaptured Hugging Face collection (`huggingface.co/collections/XiaomiMiMo/mimo-v26`) and technical-report PDF. Cheapest next ingest on this page.
