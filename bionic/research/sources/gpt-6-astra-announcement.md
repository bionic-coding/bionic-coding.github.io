...
raw_path: research/raw/2026-09-03/gpt-6-astra-announcement/
previous_captures: []
static: true
tags: [openai, gpt-6, astra, benchmarks, computer-use, coding, cybersecurity, alignment, pricing]
---

# GPT-6 Astra: A new generation of intelligence

_OpenAI launch post, 2026-09-03 (the page carries no visible dateline; the date is from the page metadata and from the sibling post index, which lists "Safety overview: GPT-6 Astra" on Sep 3, 2026). `openai.com` returns 403 to scripted fetches, so this was captured on launch day through a Playwright browser session: `source.html` is the rendered DOM (3.4 MB), `extracted.txt` the main-content text. Every benchmark figure below is OpenAI's own. The interactive bar charts were captured as axis labels only; the benchmark tables at the end of the post were captured as text and are transcribed in full._
...
> [chart, data not captured] Tabbed score-vs-API-cost charts for Terminal-Bench Science 0.1, ARC-AGI-3, FrontierMath Tier 4 (v2), Terminal-Bench 4.0, and AutomationBench, comparing GPT-6 Astra, GPT-5.6 Sol, Claude Fable 5.1, Claude Fable 5, and Claude Opus 5. Axes: API cost $0–$80, resolution rate 20–70%.
...
> [chart, data not captured] Tabs: Agents' Last Exam, ScreenSpot-Pro, OSWorld.
...
> [video demos, not captured] Circuit board · Excel competition · Game development · Filling in a Form 1040 · Frontend quality assurance · Power BI · Car transmission · Formatting a legal document.
...
> [video demos, not captured] Pediatrician search · Apartment hunting · DMV appointment · Low-carb snacks · Kindergarten analysis. Caption on the visible clip: "GPT‑6 Astra: 2 min 54 sec".
...
> [chart, data not captured] Tabs: BenchCAD, BrowseComp, OpenScore String Quartets, Design Tasks (Internal), Data Science Tasks (Internal).
...
> [slide/document demos, not captured] Gaia presentation · Spreadsheet · Document styling (reference file vs GPT‑6 Astra output).
...
> [video demos, not captured] Unreal Engine walkthrough · Blender model · Stills.
...
> [demos, not captured] Kart Racer Game · Spaceship.
...
> [demos, not captured] Career website · College search · Grocery list.
...
> [chart, data not captured] Tabs: Terminal-Bench 4.0, FrontierCode 1.1 Extended, DeepSWE, Artificial Analysis Coding Agent, Database Migration Tasks (Internal).
...
> [chart, data not captured] Tabs: GPQA Diamond, HealthBench Professional, LifeSciBench, GeneBench Pro, MedChemBench.
...
> [video demos, not captured] Sequencing quality · Cell-tracking workflow.
...
> [chart, data not captured] Tabs: ExploitBench, ExploitGym, ExploitBench (June–August 2026), SRE-Bench.
...
> [chart, data not captured] Tabs: Circumventing auto-review, ExploitGym honeypot. Caption: "This evaluation tests how models respond to auto-review denials in knowledge-work tasks. Exploiting a poor user configuration to bypass auto-review counts as failure. Astra never attempted to circumvent auto-review."
...
_Transcribed from the post's tables. Empty cells are empty on the page. Superscript footnote markers are kept as `[^n]`._

### Computer Use

| | GPT‑6 Astra | GPT‑5.6 Sol[^2] | Claude Fable 5.1 | Claude Fable 5 | Claude Opus 5 | Gemini 3.8 Flash |
|---|---|---|---|---|---|---|
...
| | GPT‑6 Astra | GPT‑5.6 Sol | Claude Fable 5.1 | Claude Fable 5 | Claude Opus 5 | Gemini 3.8 Flash |
|---|---|---|---|---|---|---|
...
| Internal computer use safety benchmark, w/ AutoReview (lower is better) | 1.8% | 4.5% | | | | |
...
| Internal hallucination benchmark (lower is better) | 2.0% | 9.4% | | | | |
...
_Numbered as on the page. The page's list skips 4; the body's marker 4 has no matching entry._
...
- Every bar chart in the body was captured as axis labels and tab names only; the numbers quoted in the prose and the end-of-post tables are the captured figures.
- All embedded videos and slide/document demos are described by their captions only.
- The in-page footnote numbering does not line up with the body markers in several places (the body cites 9 and 10 for the prime-gap results, while the HealthBench table cites 10 and the page's footnote 11 is the HealthBench note; the table's `[^7]` on FrontierCode matches footnote 8's text). Markers are transcribed as they appear on the page.
- Linked pages not captured: the Astra system card, the "safety update" ([[research/sources/openai-path-to-astra]] is the September 1 post), The Defender's Window, the Hugging Face incident post, OpenAI Daybreak, Codex Auto-review, the Sites feature page, and both prime-gap proofs.
